...
export function rule184(machine: IDGVM, zeroColor: number, oneColor: number, idx?: number){
    ...
    const left = getNeighboringPixelIndex(Direction.left, currentIDX, machine.image.width);
    const right = getNeighboringPixelIndex(Direction.right, currentIDX, machine.image.width);
    ...
    const currentMap = [CTB(leftColor), CTB(centerColor), CTB(rightColor)].toString();
