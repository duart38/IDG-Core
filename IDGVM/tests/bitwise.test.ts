...
Deno.test("OR_LIT_MEM", async function () {
    const b = makeBuilder();
    const storeAt = b.instructionIndex + 30;
    b.MoveRegisterToMemory("r3", storeAt);
    b.insert8(Instructions.BITWISE_OR);
    b.insert8(orType.OR_LIT_MEM);
    b.insert32(2);
    b.insert32(storeAt);
    assertEquals((await makeLoader(b,true)).getVM().getRegister("acc"), 3);
});
...
Deno.test("OR_MEM_LIT", async function () {
    const b = makeBuilder();
    const storeAt = b.instructionIndex + 30;
    b.MoveRegisterToMemory("r2", storeAt);
    b.insert8(Instructions.BITWISE_OR);
    b.insert8(orType.OR_MEM_LIT);
    b.insert32(storeAt);
    b.insert32(3);
    assertEquals((await makeLoader(b,true)).getVM().getRegister("acc"), 3);
});

Deno.test("OR_MEM_REG", async function () {
    const b = makeBuilder();
    const storeAt = b.instructionIndex + 30;
    b.MoveRegisterToMemory("r2", storeAt);
    b.insert8(Instructions.BITWISE_OR);
    b.insert8(orType.OR_MEM_REG);
    b.insert32(storeAt);
    b.insert32(b._regKeyToIndex("r3"));
    assertEquals((await makeLoader(b,true)).getVM().getRegister("acc"), 3);
});
...
Deno.test("XOR_REG_REG", async function () {
    const b = makeBuilder();
    b.insert8(Instructions.BITWISE_OR);
    b.insert8(orType.XOR_REG_REG);
    b.insert32(b._regKeyToIndex("r2"));
    b.insert32(b._regKeyToIndex("r3"));
    assertEquals((await makeLoader(b,true)).getVM().getRegister("acc"), 1);
});

Deno.test("XOR_LIT_MEM", async function () {
    const b = makeBuilder();
    const storeAt = b.instructionIndex + 30;
    b.MoveRegisterToMemory("r3", storeAt);
    b.insert8(Instructions.BITWISE_OR);
    b.insert8(orType.XOR_LIT_MEM);
    b.insert32(2);
    b.insert32(storeAt);
    assertEquals((await makeLoader(b,true)).getVM().getRegister("acc"), 1);
});